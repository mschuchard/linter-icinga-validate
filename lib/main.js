    ...
    return {
      ...
      lint: (activeEditor) => {
        ...
        if (!(atom.config.get('linter-icinga-validate.passwordlessSudo'))) {
          ...
          args.unshift('-S');
          // provide password with stdin; does not work probably because stdin not being pass in correctly by sb-exec
          opts.stdin =  atom.config.get('linter-icinga-validate.sudoPassword') + "\n";
